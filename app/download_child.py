...
from app.config import MAX_ATTEMPTS, RETRY_DELAYS
...
from app.engine.matcher import MusicMatcher, proxy_with_credentials
...
def run(item_id: str) -> int:
    ...
    try:
        ...
        match = MusicMatcher().find_match(song, settings)
        with transaction(immediate=True) as conn:
            conn.execute(
        ...
        if code == "DISK_FULL":
            ...
            return 1
        if attempt < MAX_ATTEMPTS and code not in {"DISK_FULL", "FILESYSTEM_PERMISSION_ERROR"}:
            ...
            details = redact_secrets(exc)[:1500]
            _set_state(item, "FAILED", 100, error_code=code, error=f"{friendly}\n\nTechnical details: {details}")
        update_job_counts(item["job_id"])
